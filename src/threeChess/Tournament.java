...
import threeChess.agents.AgentBrutus;
...
public class Tournament {
  ...
  private static final int SECONDS_PER_REPORT = 5;
  ...
  /**
   * @param agents an array of agents to be tested. Must have at least 3 agents.
   * @param numGames the number of games to play amongst all agents.
   * @param timeLimitSeconds the time limit for each agent in each game.
   * @param maximumTurns the maximum number of turns to play until calling the game quits.
   * @param pauseMS the minimum amount of time to wait between each move.
   * @param threads the number of threads to run games on.
   * @param logger the logger to log the output of the tournament to.
   * @param displayOn whether to have a display for the games.
   */
  public Tournament(
  ...
  public void runTournament() {
    ...
    for (Game game : gamesToPlay) {
      ...
      for (Colour colour : Colour.values()) {
        ...
        double meanPly = 0;
        if (agent instanceof AgentBrutus) {
          AgentBrutus brutus = (AgentBrutus) agent;
          meanPly = (double) brutus.plySum / brutus.moveCount;
        } else if (agent instanceof AgentFixedPly) {
          meanPly = ((AgentFixedPly) agent).ply;
        }
        agentStats.update(score, status, time, meanPly);
      }
    }
  ...
  public static class AgentStats implements Comparable<AgentStats> {
    ...
    public void update(int score, Game.GameStatus status, int timeMS, double meanPly) {
      games += 1;
    ...
    @Override public String toString() {
      ...
      double ply = Math.round((double) meanPlySum / games * 100.0d) / 100.0d;
      return pad(agent.toString(), 20, 3)
          ...
          + pad("mean-ply: " + ply, 18, 2);
    }

    @Override public int compareTo(AgentStats other) {
