...
/**
 * Game constants and logic used for fast simulation and evaluation of game states.
 *
 ...
 * - Use of numbers always instead of the PieceType or Colour enums, as numbers are faster than references.
 *
 * Some of the micro-optimisations used are explained in
...
public class GameLogic {
  ...
  /** Counts the total number of moves that have been evaluated. **/
  ...
  public static final class GameState {
    ...
    /** Computes the list of capturing moves that are available from this state into {@param capturingMoves}. **/
    public final void computeCapturingMoves(IdentitySet<Move> capturingMoves) {
      ...
      for (int index = 0; index < 96 /* totalSquares */; ++index) {
        ...
        for (int moveIndex = 0; moveIndex < length; ++moveIndex) {
          ...
          // We are only concerned with capturing moves.
          byte toPiece = pieces[move.toIndex];
          if (toPiece == 0)
            continue;
          int toColour = toPiece & 3;
          ...
          if (move instanceof MoveMany) {
            ...
            if (toColour == colour)
              continue;
          } else if (toColour == colour || !move.isValidMove(this))
            continue;
  ...
  public static class CombinedGameConstants extends GameConstants implements Cloneable {
    ...
    /** @return a clone of these game constants so that they can be used between many threaded games. **/
    @Override public CombinedGameConstants clone() {
      return new CombinedGameConstants(start, end);
    }
    ...
    /** @return a random set of combined game constants. **/
    public static CombinedGameConstants createRandom() {
      GameConstants start = GameConstants.random();
      GameConstants end = GameConstants.random();
      return new CombinedGameConstants(start, end);
    }

    /** @return a new combined game constants that is mutated randomly from one and two. **/
    public static CombinedGameConstants mutate(CombinedGameConstants one, CombinedGameConstants two) {
      return new CombinedGameConstants(
          GameConstants.mutate(one.start, two.start),
          GameConstants.mutate(one.end, two.end)
      );
    }
  ...
  public static class GameConstants {
    ...
    /** @return a random set of constants. **/
    public static GameConstants random() {
      return new GameConstants(
          1 + random.nextInt(20), // selfWeight
          new double[] {
              rand(1, 15), // Pawn
              rand(1, 30), // Knight
              rand(1, 30), // Bishop
          ...
          rand(1, 10), // pawnRowValue
          rand(1, 10) // moveCountValue
      );
    }

    /** @return a set of constants randomly selected between one and two. **/
    public static GameConstants mutate(GameConstants one, GameConstants two) {
      double[] typeValues = new double[6];
      for (int index = 0; index < 6; ++index) {
        typeValues[index] = mix(one.typeValues[index], two.typeValues[index]);
      }
      return new GameConstants(
          (int) Math.round(mix(one.selfWeight, two.selfWeight)),
          typeValues, mix(one.pawnRowValue, two.pawnRowValue),
          mix(one.moveCountValue, two.moveCountValue)
      );
    }
    ...
    /** @return a string representation of these game constants. **/
    @Override public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append(selfWeight).append(", ");
      ...
      builder.append("}, ");
      builder.append(pawnRowValue).append(", ");
      builder.append(moveCountValue);
  ...
  public static abstract class PawnMove extends Move {
    ...
    public static List<Move> getPotentialMoves(int fromIndex, Colour colour) {
      ...
      pawnMove = (reverse ? Direction.reverse(pawnMove) : pawnMove);
  ...
  /** @return randomly number between one or two. **/
  public static double mix(double one, double two) {
    double mix = random.nextDouble();
    return mix * one + (1 - mix) * two;
  }
